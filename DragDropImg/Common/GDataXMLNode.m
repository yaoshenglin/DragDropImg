...
+ (id)nodeBorrowingXMLNode:(xmlNodePtr)theXMLNode {
    ...
    xmlCount++;
    if (theXMLNode->ns!=NULL) {
        theXMLNode->ns=NULL;
        NSLog(@"%d.中华人民共和国",xmlCount);
    }
...
- (NSString *)stringFromXMLString:(const xmlChar *)chars
{
    ...
    if (cacheDict) {
        ...
        CFDictionarySetValue(cacheDict, chars, ( const void *)(result));
    }
...
- (NSString *)name
{
    ...
    NSError *error=nil;
    NSString *Path=@"~/Documents/name.txt";
...
//        NSString *str = [NSString string];tewt
        if (localName != NULL) {
            if (prefix != NULL) {
...
- (void)addStringsCacheToDoc {
    ...
    CFMutableDictionaryRef dict = CFDictionaryCreateMutable(kCFAllocatorDefault, capacity, &keyCallBacks, &kCFTypeDictionaryValueCallBacks);
